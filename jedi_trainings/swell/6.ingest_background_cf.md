...
models:
  geos_cf:
    # strftime template for the source files. The default already points at the
    # GEOS-CFv2 jdi collection, so you normally do not need to set this.
...
│       └── interfaces/
│           └── geos_cf/
│               └── model/                 ← model interface, including the R2D2 fetch/store keys
└── <experiment_id>-suite/
    ├── flow.cylc
